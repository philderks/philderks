Hey, I’m Philipp. I build things for the web, manage my own servers, and occasionally play the games I write tools for.

![location](https://img.shields.io/badge/zürich%2C_switzerland-→_derks.dev-f97316?style=flat-square&labelColor=141414&link=https://derks.dev)

### Skills

[![My Skills](https://skillicons.dev/icons?i=nextjs,vue,js,html,css,python,java,spring,mysql,docker,aws,cloudflare,vercel,linux,bash,git,npm,vscode,obsidian,unreal&perline=10)](https://skillicons.dev)

-----

### Currently building

- [derks.dev](https://derks.dev) — personal portfolio
- [Fabricator](https://fabricator.site/) — Minecraft server manager built with Flask + Vue 3
- [Homelab](https://github.com/philderks/Homelab) — self-hosted infrastructure, documented

-----

<img src="https://github-readme-stats.vercel.app/api?username=philderks&show_icons=true&hide_border=true&bg_color=141414&title_color=f97316&text_color=e8e0d5&icon_color=f97316&rank_icon=github" />
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=philderks&layout=compact&hide_border=true&bg_color=141414&title_color=f97316&text_color=e8e0d5&langs_count=5" />

-----

[derks.dev](https://derks.dev) · [linkedin](https://linkedin.com/in/philipp-derks)
